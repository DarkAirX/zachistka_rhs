...
private ["_position_array","_radarray","_numobjs","_campmark","_locselname","_objlist","_commanderlist","_transport_heli_list","_ammobox_list","_ghst_side","_PARAM_AISkill","_o","_random_pos","_buildarray","_curtasks","_max_reinf"];
...
While {true} do {
	...
	_randnum = (random 10);
	sleep 10;
	if ((_ghst_side countSide allunits < 35) and (_randnum > 9) and (count _curtasks > 0) and (_max_reinf < _numobjs)) then {[_random_pos,_radarray,_transport_heli_list,false,[false,"ColorRed"],(_PARAM_AISkill/10)] spawn ghst_fnc_eparadrop; _max_reinf = _max_reinf + 1;
	};
	if ((_ghst_side countSide allunits < 35) and (_randnum < 2) and (count _curtasks > 0) and (_max_reinf < _numobjs)) then {[_random_pos,[2000,2000],(_radarray select 0),(1 + (ROUND (random 2))),[false,"ColorRed"],(_PARAM_AISkill/10)] spawn ghst_fnc_ecounter; _max_reinf = _max_reinf + 1;
	};
	sleep 10;
};